...
async function run() {
    ...
    for (let i = 0; i < seedPhrases.length; i++) {
        ...
        const nodeMaps: Record<string, { id: string; text: string }> = {};
        ...
        for (const targetLocale of targetLocales) {
            ...
            try {
                ...
                const res = await translateCore(db, seed.text, seed.locale, targetLocale, 0.8, true);
                if (res) {
    ...
    await rebuildGlobalCacheLogic(db, undefined, seedPhrases.length);
