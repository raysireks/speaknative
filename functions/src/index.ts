 ...
 */
export const getSimilarPhrases = functions.https.onCall(async (request) => {
    ...
    let query: admin.firestore.Query = phrasesRef;
    ...
    if (userLocale && matches.length > 0) {
        const conceptIds = matches.map((m: any) => m.concept_id).filter(Boolean);
        ...
        if (conceptIds.length > 0) {
            ...
            return matches.map((m: any) => {
                ...
                return {
                    id: m.concept_id, // Use concept_id as the stable ID
                    text: m.text,
                    translation: userText,
                    is_slang: m.is_slang || false,
                    slangText: m.is_slang ? m.text : undefined, // For backwards compat / UI ease
                    slangTranslation: undefined,
                    ...m,
                    embedding: undefined
                };
            });
...
export const translateAndStore = functions.https.onCall(async (request) => {
    ...
    if (userDocSnapshot.empty) {
        ...
        const newUserDoc = {
            text: text,
            is_slang: false, // Assumed false for raw input unless analyzed
            usage_count: 1,
            locale: userLocale,
    ...
    if (!targetSnapshot.empty) {
        ...
        return {
            id: docRef.id,
            ...newTargetDoc,
            embedding: undefined, // Hide large vector in response
            source: 'generated'
        };
    }
});
    ...
    for (const sourceLocale of SUPPORTED_LOCALES) {
        ...
        for (const doc of topSnapshot.docs) {
            ...
            const phraseEntry: any = {
                ...
                is_slang: data.is_slang, // Include slang flag
                translations: {}
            };
