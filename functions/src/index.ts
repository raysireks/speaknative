...
import { translateCore } from './translate_core';
    ...
    if (userLocale && matches.length > 0) {
        const resultsWithTranslation = await Promise.all(matches.map(async (m) => {
            ...
            return {
                ...
                ...m,
                embedding: undefined // Remove large vector
            };
        }));
...
export const getUnifiedPhraseCache = functions.https.onCall(async (request) => {
    ...
    async function performSearch() {
        ...
        let translatedMap: Record<string, boolean> = {};
        if (typeof sourceData.translated === 'boolean' && sourceData.translated) {
            translatedMap = SUPPORTED_LOCALES.reduce((acc, loc) => ({ ...acc, [loc]: true }), {});
        ...
        for (const targetLoc of SUPPORTED_LOCALES) {
            ...
            const matches = candidates.filter(d => d.data().locale === targetLoc).map(d => {
                const dA = d.data();
                const v = getVectorData(dA.embedding);
    ...
    if (searchResult.missingLocales && searchResult.missingLocales.length > 0) {
        ...
        const generatePromises = searchResult.missingLocales.map((loc: string) =>
            translateCore(db, text, userLocale, loc, Number(threshold))
                .catch((e: Error) => {
